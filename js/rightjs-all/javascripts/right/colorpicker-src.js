    ...
    var Colorpicker = new Widget({
        ...
        initialize: function (options) {
            ...
            if (this.options.update) {
                this.assignTo(this.options.update, this.options.trigger);
            }
            if (this.options.updateBg) {
                this.updateBg(this.options.updateBg);
            }
            if (this.options.updateBorder) {
                this.updateBorder(this.options.updateBorder);
            }
            if (this.options.updateColor) {
                this.updateColor(this.options.updateColor);
            }
...
// protected
        ...
        update: function () {
            this.field._.style.backgroundColor = 'rgb(' + this.tint.map(function (c) {
        ...
        initialize: function () {
            ...
            this.insert([
                ...
                $E('div', {'class': 'rgb-display'}).insert([
                    $E('div').insert([$E('label', {html: 'R:'}), this.rDisplay = $E('input', {
                        maxlength: 3,
                        cIndex: 0
                    })]),
                    $E('div').insert([$E('label', {html: 'G:'}), this.gDisplay = $E('input', {
                        maxlength: 3,
                        cIndex: 1
                    })]),
                    $E('div').insert([$E('label', {html: 'B:'}), this.bDisplay = $E('input', {
                        maxlength: 3,
                        cIndex: 2
                    })])
                ]),
    ...
    Colorpicker.include({
        ...
        toHex: function (color) {
            return '#' + this.color.map(function (c) {
         ...
         */
        color2tint: function () {
            var color = $A(this.color).sort(function (a, b) {
                    return a - b;
                }),
