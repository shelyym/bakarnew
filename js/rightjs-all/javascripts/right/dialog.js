 ...
 */
var Dialog = RightJS.Dialog = function (a) {
    function o(a) {
        m.dragged && m.dragged.head.dragStop(a)
    }

    function n(a) {
        m.dragged && m.dragged.head.dragMove(a)
    }
    ...
    }), d = new a.Class(a.Element, {
        initialize: function (b) {
            this.$super("div", {"class": "rui-spinner"}), this.dots = [];
            for (var c = 0; c < (b || 4); c++)this.dots.push(new a.Element("div"));
            this.dots[0].addClass("glowing"), this.insert(this.dots), a(this.shift).bind(this).periodical(300)
        }, shift: function () {
            if (this.visible()) {
                var a = this.dots.pop();
    ...
    }), e = a, f = a.$, g = a.$w, h = a.$E, i = a.Xhr, j = a.Class, k = a.Object, l = a.Element, m = new b({
        extend: {
            version: "2.2.2",
            EVENTS: g("ok cancel help expand collapse resize load"),
            Options: {
                lockScreen: !0,
                fxDuration: "short",
                draggable: !0,
            ...
            this.$super("dialog", a).append(this.head = new m.Head(this), this.body = new m.Body(this), this.foot = new m.Foot(this)).onCancel(this.hide), this.locker = h("div", {"class": "rui-screen-locker"}), this.options.title && this.title(this.options.title), this.options.html && this.html(this.options.html), this.options.url && this.load(this.options.url)
        }, show: function () {
            this.options.lockScreen && this.locker.insertTo(document.body), this.setStyle("visibility:hidden").insertTo(document.body).resize().setStyle("visibility:visible;opacity:0"), this.options.fxDuration ? this.morph({opacity: 1}, {duration: this.options.fxDuration}) : this.setStyle("opacity:1");
            return m.current = this
        }, hide: function () {
            this.locker.remove(), this.remove(), m.current = !1;
            return this
        }, resize: function () {
            arguments.length && this.$super.apply(this, arguments);
            var a = this.size(), b = f(window).size();
            this.expanded && (a.x = b.x - 20, a.y = b.y - 10, this.$super.call(this, a)), this.setStyle({
                top: (b.y - a.y) / 2 + f(window).scrolls().y + "px",
                left: (b.x - a.x - 16) / 2 + "px"
            });
            return this.fire("resize")
        }, title: function (a) {
            if (arguments.length) {
                this.head.title.html(a);
                return this
            }
            ...
            return arguments.length ? this.$super.apply(this, arguments) : this.body.html()
        }, load: function (a, b) {
            this.show(), this.body.load(a, b);
            return this
        }, expand: function () {
            this.expanded || (this._prevSize = this.size(), this.resize({
                x: f(window).size().x - 20,
                y: f(window).size().y - 10
            }), this.expanded = !0, this.fire("expand"));
            return this
        }, collapse: function () {
            this.expanded && (this.expanded = !1, this.resize(this._prevSize), this.fire("collapse"));
            return this
        }
    });
    m.Head = new j(l, {
        initialize: function (a) {
            this.dialog = a, this.options = a.options, this.$super("div", {"class": "rui-dialog-head"}), this.append(this.icon = h("div", {"class": "icon"}), this.title = h("div", {
                "class": "title",
                html: "&nbsp;"
            }), this.tools = h("div", {"class": "tools"})), this.fsButton = h("div", {
                "class": "expand",
                html: "&equiv;",
                title: m.i18n.Expand
            }).onClick(function () {
                a.expanded ? (a.collapse(), this.html("&equiv;").set("title", m.i18n.Expand)) : (a.expand(), this.html("_").set("title", m.i18n.Collapse))
                ...
                a.fire("cancel")
            }), this.options.expandable && this.tools.insert(this.fsButton), this.options.closeable && this.tools.insert(this.closeButton), this.on({
                selectstart: function (a) {
                    a.stop()
                }, mousedown: this.dragStart, touchstart: this.dragStart
            }), this.options.draggable || this.dialog.addClass("rui-dialog-nodrag")
        }, dragStart: function (a) {
            if (this.options.draggable && !a.find("div.tools div")) {
                var b = this.dialog.dimensions(), c = a.position();
                this.xDiff = b.left - c.x, this.yDiff = b.top - c.y, this.maxX = f(window).size().x - b.width - 20, this.dlgStyle = this.dialog.get("style"), m.dragged = this.dialog, a.stop()
            }
        }, dragMove: function (a) {
            var b = a.position(), c = b.x + this.xDiff, d = b.y + this.yDiff;
            c < 0 ? c = 0 : c > this.maxX && (c = this.maxX), d < 0 && (d = 0), this.dlgStyle.top = d + "px", this.dlgStyle.left = c + "px"
        }, dragStop: function (a) {
            m.dragged = !1
        }
    }), m.Body = new j(l, {
        initialize: function (a) {
            this.dialog = a, this.options = a.options, this.$super("div", {"class": "rui-dialog-body"}), this.locker = h("div", {"class": "rui-dialog-body-locker"}).insert(new d)
        }, load: function (a, b) {
            this.insert(this.locker, "top"), this.xhr = (new i(a, k.merge({method: "get"}, b))).onComplete(e(function (a) {
                this.update(a.text), this.dialog.resize().fire("load")
            }).bind(this)).send();
            return this
        }, update: function (a) {
            this.$super(a), this.options.showIcon && this.insert('<div class="rui-dialog-body-icon">' + this.options.showIcon + "</div>", "top");
            return this
        }
    }), m.Foot = new j(l, {
        initialize: function (a) {
            this.$super("div", {"class": "rui-dialog-foot"}), this.dialog = a, a.okButton = (new c(m.i18n.Ok, {"class": "ok"})).onClick(function () {
                a.fire("ok")
            }), a.helpButton = (new c(m.i18n.Help, {"class": "help"})).onClick(function () {
                a.fire("help")
            }), a.cancelButton = (new c(m.i18n.Cancel, {"class": "cancel"})).onClick(function () {
                a.fire("cancel")
            }), a.options.showHelp && this.insert(a.helpButton), a.options.closeable && this.insert(a.cancelButton), this.insert(a.okButton)
        }
    }), m.Alert = new j(m, {
        initialize: function (a) {
            a = k.merge({
                showIcon: "!",
                title: m.i18n.Alert
            }, a), this.$super(a), this.addClass("rui-dialog-alert"), this.on("ok", "hide")
        }
    }), m.Confirm = new j(m, {
        initialize: function (a) {
            a = k.merge({
                showIcon: "?",
                title: m.i18n.Confirm
            }, a), this.$super(a), this.addClass("rui-dialog-confirm"), this.on("ok", "hide")
        }
    }), m.Prompt = new j(m, {
        initialize: function (b) {
            b = k.merge({
