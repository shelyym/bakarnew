...
// adds XHR events to the element
    function add_xhr_events(element, options) {
        return Object.merge({
            ...
            onSuccess: function () {
                element.fire('ajax:success', {xhr: this});
            },
            onFailure: function () {
                element.fire('ajax:failure', {xhr: this});
            }
        }, options);
    }
...
// global events listeners
    ...
    var RR = {
        ...
        insert: function (where, what, in_position) {
            ...
            switch (position) {
                case 'bottom':
                    new_element = container.children().last();
                    break;
                case 'top':
                    new_element = container.first();
                    break;
                case 'before':
                    new_element = container.prev();
                    break;
                case 'after':
                    new_element = container.next();
                    break;
            }
