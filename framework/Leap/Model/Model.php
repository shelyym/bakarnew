...
abstract class Model {
	...
	/*
        ...
        * CRUD Setting
        */
       public $crud_setting = array("add"=>1,"search"=>1,"viewall"=>1,"export"=>1,"toggle"=>1,"import"=>0,"webservice"=>0,"publish"=>0,"publish_flat"=>0,"refresh"=>0);
    ...
    public $loadReadHook = ""; //utk hook waktu read
...
//		echo $q;
	...
	public function read ($perpage = 20)
	{
		...
		if (count($this->read_filter_array) > 0) {
			foreach ($arrClms as $col) {
				if (array_key_exists($col, $this->read_filter_array)) {
                    if($this->read_filter_array[$col] === "notnull"){
                        $imp2[] = " $col != '' ";
//                        echo "in";
//                        echo " $col != '' ";
//                        echo $this->read_filter_array[$col];
                    }else {
...
//		$return['colomslist'] = $column;
	 ...
	 * return all no where
	 */
	public function getAll ($sort = "", $limit = "", $selectedColom = "")
	{
        if($selectedColom == "")$readtext = $this->default_read_coloms;
        else $readtext = $selectedColom;

		global $db;
		$q = "SELECT $readtext FROM {$this->table_name} $sort $limit";
		$arr = $db->query($q, 2);
