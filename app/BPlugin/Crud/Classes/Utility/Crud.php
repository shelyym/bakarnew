...
//                            }
    ...
    public static function clean2print($obj,$arr,$strip_tag=1){ //1 strip tag //2 utf_encode , 0 no action
...
//        $main_id = $this->main_id;
        ...
        foreach($arr as $o){
            ...
            foreach($exp as $attr){
                ...
                if($strip_tag == 1)
                    $sem[$attr] = AppContentHelper::stripTag(stripslashes ($o->$attr));
                elseif($strip_tag == 2)
                    $sem[$attr] =utf8_encode(stripslashes ($o->$attr));
                else
                    $sem[$attr] = stripslashes ($o->$attr);

                if(is_numeric($o->$attr)){
//                    echo "numeric";
                    $int = doubleval($o->$attr);
                    $o->$attr = $int;
//                    echo $attr.": ".$int."<br>";
                }
            }
...
//        $main_id = $this->main_id;
        ...
        foreach($exp as $attr){
            $sem[$attr] = AppContentHelper::stripTag(stripslashes ($obj->$attr));
        }
    ...
    public static function clean2printEinzelnWithReferenz($obj,$objref){

//        $this->default_read_coloms = $this->crud_webservice_allowed;
//        $main_id = $this->main_id;
        $exp = explode(",",$objref->crud_webservice_allowed);
    ...
    public static function clean2printEinzelnWithColoumsWithStriptags($obj){
...
//        $main_id = $this->main_id;
    ...
    public static function clean2printWithColom($obj,$arr){
...
//        $main_id = $this->main_id;
    ...
    public static function PublishButton ($return, $webClass)
    {
        ...
        <button class="btn btn-default" id="<?= $c; ?>publish<?= $t; ?>" type="button"><?= Lang::t('publish'); ?></button>

        <script type="text/javascript">
            $("#<?=$c;?>publish<?=$t;?>").click(function () {

                var win = window.open('<?=_BPATH;?>CrudPublisher/UploadJS2?cname=<?=$c;?>', '_blank');
                win.focus();


//                openInNewTab2();
//                $.post("<?//=_SPPATH;?>//CrudPublisher/publishTofirebase",{cname:"<?//=$c;?>//"},function(data){
//                    alert("finish"+data);
//                },'json');
            });
    ...
    public static function PublishButtonFlat ($return, $webClass)
    {
        ...
        <button class="btn btn-default" id="<?= $c; ?>publishflat<?= $t; ?>" type="button"><?= Lang::t('publish flat'); ?></button>

        <script type="text/javascript">
            $("#<?=$c;?>publishflat<?=$t;?>").click(function () {

                var win = window.open('<?=_BPATH;?>CrudPublisher/UploadJS2Flat?cname=<?=$c;?>', '_blank');
                win.focus();
...
//                },'json');
    ...
    public static function RefreshButton ($return, $webClass)
    {
        ...
        <button class="btn btn-default" id="<?= $c; ?>refresh<?= $t; ?>" type="button"><?= Lang::t('Fetch'); ?></button>

        <script type="text/javascript">
            $("#<?=$c;?>refresh<?=$t;?>").click(function () {

                $.get("<?=_BPATH.$model->refreshUrl;?>",function(data){
                    alert(data);
                });
